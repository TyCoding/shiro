...
//操作-权限按钮
//此处的权限按钮功能：1、用来展示当前角色所关联的权限信息，以ZTree树的方式展示，由后端构建ZTree树的JSON数据结构，目的是实现对当前角色的权限管理。
//                 2、为了维护角色-权限表的关系。之前在用户层面上需要维护用户-角色表关系；现在在角色层面上需要维护角色-权限关系。
...
function edit(id) {
    ...
    if (id != null || id != '') {
        ...
        var roles = findById(id);
        $('#edit-modal .modal-body .role').val(roles.role);
        $('#edit-modal .modal-body .description').val(roles.description);
        $('#edit-modal .modal-body .id').val(roles.id);
    }
}
